...
class Beam():
    ...
    def __init__(self, size, device=False):
        ...
        self.scores = tf.zeros([size,], dtype = tf.float32)
        self.all_scores = []
        ...
        self.next_ys = [tf.fill([size,], config.PAD_idx, dtype=tf.int64)]
    ...
    def advance(self, word_prob):
        ...
        if len(self.prev_ks) > 0:
            ...
            beam_lk = word_prob + tf.reshape(tf.expand_dims(self.scores, 1), word_prob.shape)
        else:
            beam_lk = word_prob[0]

        #flat_beam_lk = beam_lk.view(-1)
        flat_beam_lk = tf.reshape(-1)
    ...
    def sort_scores(self):
        ...
        return tf.sort(self.scores, 0, direction='DESCENDING')
...
class Translator(object):
    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        def collect_active_part(beamed_tensor, curr_active_inst_idx, n_prev_active_inst, n_bm):
            ...
            beamed_tensor = tf.reshape(beamed_tensor, -1)
            beamed_tensor = beamed_tensor.index_select(0, curr_active_inst_idx)
            #beamed_tensor = beamed_tensor.view(*new_shape)
            beamed_tensor = tf.reshape(beamed_tensor, *new_shape)
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, DB_ext_vocab_batch):
            ...
            def prepare_beam_dec_seq(inst_dec_beams, len_dec_seq):
                ...
                dec_partial_seq = tf.stack(dec_partial_seq)
                #dec_partial_seq = dec_partial_seq.view(-1, len_dec_seq)
                dec_partial_seq = tf.reshape(dec_partial_seq, [-1, len_dec_seq])
                return dec_partial_seq

            def prepare_beam_dec_pos(len_dec_seq, n_active_inst, n_bm):
                #dec_partial_pos = torch.arange(1, len_dec_seq + 1, dtype=torch.long, device=self.device)
                dec_partial_pos = tf.range(1, len_dec_seq + 1, dtype = tf.int64)
                #dec_partial_pos = dec_partial_pos.unsqueeze(0).repeat(n_active_inst * n_bm, 1)
                dec_partial_pos = tf.tile(tf.expand_dims(dec_partial_pos, 0), [n_active_inst * n_bm, 1])
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src, encoder_db, mask_transformer_db, DB_ext_vocab_batch):
                ...
                mask_trg = tf.expand_dims(tf.math.equal(dec_seq, config.PAD_idx), axis = 1)
                
                #mask_src = torch.cat([mask_src[0].unsqueeze(0)]*mask_trg.size(0),0)
                mask_src = tf.concat([tf.expand_dims(mask_src[0], 0)]*mask_trg.shape[0],0)
                ...
                word_prob = tf.reshape(word_prob, [n_active_inst, n_bm, -1])
                return word_prob
        ...
        src_seq = tf.reshape(tf.tile(enc_batch, [1, n_bm]), [n_inst * n_bm, len_s])
        #src_enc = src_enc.repeat(1, n_bm, 1).view(n_inst * n_bm, len_s, d_h)
        src_enc = tf.reshape(tf.tile(src_enc, [1, n_bm, 1]), [n_inst * n_bm, len_s, d_h])
...
def sequence_mask(sequence_length, max_len=None):
    ...
    seq_range = tf.cast(tf.range(0, max_len), dtype = tf.int64)
    #seq_range_expand = seq_range.unsqueeze(0).expand(batch_size, max_len)
    seq_range_expand = tf.tile(tf.expand_dims(seq_range, 0), [batch_size,1])
    print(seq_range_expand)
    seq_range_expand = seq_range_expand
    if sequence_length.is_cuda:
        seq_range_expand = seq_range_expand.cuda()
    #seq_length_expand = (sequence_length.unsqueeze(1).expand_as(seq_range_expand))
    seq_length_expand = tf.broadcast_to(tf.expand_dims(sequence_length, 1),.seq_range_expand)
...
def get_input_from_batch(batch):
    ...
    if config.is_coverage:
        ...
        coverage = tf.zeros(enc_batch.size())
