...
def read_langs(vocab):
    ...
    train_context = np.load(
        "empathetic-dialogue/sys_dialog_texts.train.npy", allow_pickle=True
    )
    train_target = np.load(
        "empathetic-dialogue/sys_target_texts.train.npy", allow_pickle=True
    )
    train_emotion = np.load(
        "empathetic-dialogue/sys_emotion_texts.train.npy", allow_pickle=True
    )
    train_situation = np.load(
        "empathetic-dialogue/sys_situation_texts.train.npy", allow_pickle=True
    )

    dev_context = np.load(
        "empathetic-dialogue/sys_dialog_texts.dev.npy", allow_pickle=True
    )
    dev_target = np.load(
        "empathetic-dialogue/sys_target_texts.dev.npy", allow_pickle=True
    )
    dev_emotion = np.load(
        "empathetic-dialogue/sys_emotion_texts.dev.npy", allow_pickle=True
    )
    dev_situation = np.load(
        "empathetic-dialogue/sys_situation_texts.dev.npy", allow_pickle=True
    )

    test_context = np.load(
        "empathetic-dialogue/sys_dialog_texts.test.npy", allow_pickle=True
    )
    test_target = np.load(
        "empathetic-dialogue/sys_target_texts.test.npy", allow_pickle=True
    )
    test_emotion = np.load(
        "empathetic-dialogue/sys_emotion_texts.test.npy", allow_pickle=True
    )
    test_situation = np.load(
        "empathetic-dialogue/sys_situation_texts.test.npy", allow_pickle=True
    )
    ...
    assert (
        len(data_train["context"])
        == len(data_train["target"])
        == len(data_train["emotion"])
        == len(data_train["situation"])
    )
    ...
    assert (
        len(data_dev["context"])
        == len(data_dev["target"])
        == len(data_dev["emotion"])
        == len(data_dev["situation"])
    )
    ...
    assert (
        len(data_test["context"])
        == len(data_test["target"])
        == len(data_test["emotion"])
        == len(data_test["situation"])
    )
...
def load_dataset(print_sample=True):
    if os.path.exists("empathetic-dialogue/dataset_preproc.p"):
        ...
        data_tra, data_val, data_tst, vocab = read_langs(
            vocab=Lang(
                {
                    config.UNK_idx: "UNK",
                    config.PAD_idx: "PAD",
